# this is the emails.csv file code 2-22
import torch
import pandas as pd
...
smaller_dataset = pd.read_csv("emails_rationale.csv", encoding='latin1')

# X number of samples from each class
few_shot_samples = pd.concat([smaller_dataset[smaller_dataset['spam'] == 0].sample(n=num_shot, random_state=random_state),
                           smaller_dataset[smaller_dataset['spam'] == 1].sample(n=num_shot, random_state=random_state)])
...
labels = few_shot_samples['spam'].tolist()
...
test_data = pd.read_csv("emails.csv", encoding='latin1')
print(f"Test Data size: ", test_data.shape)

# Keep only the desired columns
test_data = test_data[['text', 'spam']]
# Remove rows with None values in the 'Text' column
test_data = test_data.dropna(subset=['text'])
test_data = test_data.dropna(subset=['spam'])

class_0_data = test_data[test_data['spam'] == 0]
class_1_data = test_data[test_data['spam'] == 1]

# Ensure both classes have the same number of samples
min_samples = min(len(class_0_data), len(class_1_data))

# Sample an equal number of samples from both classes
balanced_test_data = pd.concat([
    class_0_data.sample(n=min_samples, random_state=random_state),
    class_1_data.sample(n=min_samples, random_state=random_state)
])
...
percentage = 0.10
test_data = balanced_test_data.sample(frac=percentage, random_state=random_state)
...
test_labels = test_data['spam'].tolist()
...
# # Print the sorted list
# print("\nSorted Test Accuracies:")
# for acc, epoch in sorted(test_accuracies, key=lambda x: x[0], reverse=True):
#     print(f"Epoch {epoch}: Test Accuracy = {acc:.4f}")
#
# print("\nSorted F1s:")
# for f1, epoch in sorted(f1_scores, key=lambda x: x[0], reverse=True):
#     print(f"Epoch {epoch}: F1 Score = {f1:.4f}")
#
# print(f"Total execution time: {total_time:.2f} seconds")
